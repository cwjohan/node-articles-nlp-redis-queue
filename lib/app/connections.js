...
function Connector(mongoUrl, redisUrl) {
  ...
  var queueBroker = this.queueBroker = new WorkQueueBroker('../redis-queue-config.json');
  queueBroker.connect(function() {
    logger.log({ type: 'info', msg: 'connected', service: 'node-redis-queue' });
    queueBroker.on('error', function(err) {
      logger.log({ type: 'error', msg: err, service: 'node-redis-queue' });
    });
    queueBroker.on('end', function() {
